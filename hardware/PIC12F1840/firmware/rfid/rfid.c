...
#define RFID_OUT_D					pda.d2
#define RFID_INPUT					pia.i0
#define RFID_INT_F					intext.iocaf.iocaf0
#define RFID_I_RIE					intext.iocap.iocap0
#define RFID_I_FIE					intext.iocan.iocan0
#else
#define RFID_OUT_D					pda.d5
#define RFID_INPUT					pia.i4
#define RFID_INT_F					intext.iocaf.iocaf4
#define RFID_I_RIE					intext.iocap.iocap4
#define RFID_I_FIE					intext.iocan.iocan4
#endif

#define PERIOD						(uint8_t)(_XTAL_FREQ / (COMMAND_CYCLE * RFID_FREQ) - 1)
#define RFID_DUTY					((PERIOD+1)<<1)

#define CAPT_TMR_DIV				4
#define CAPT_TMR_LSB_US				(_XTAL_FREQ/(COMMAND_CYCLE*CAPT_TMR_DIV*US_PER_SECOND))
...
#define PWM_INIT()					{RFID_OUT_D    = OUTPUT;                  \
									 timer2.pr2    = PERIOD;                  \
									 ccp1.ccpr1l   = (RFID_DUTY >> 2) & 0x3F; \
									 ccp1.dc1b     = RFID_DUTY & 0x03;        \
									 ccp1.ccp1m    = CCP1M_PWM_P1AH_P1BH;}
#define CAPT_INIT()					{timer1.ckps   = T1_CKPS_COEF_4; \
									 timer1.tmr1   = 0;              \
									 RFID_I_RIE    = TRUE;           \
									 RFID_I_FIE    = TRUE;           \
									 intcon.gie    = TRUE;           \
...
#define FIELD_ON()					{timer2.on     = TRUE;  \
									 RFID_OUT_D    = OUTPUT;}
#define FIELD_OFF()					{timer2.on     = FALSE; \
									 RFID_OUT_D    = INPUT;}

#define CAPTURE_DISABLE()			intcon.iocie   = FALSE
#define CAPTURE_ENABLE()			intcon.iocie   = TRUE
#define CAPTURE_CLEAR()				{RFID_INT_F    = FALSE; \
									 intcon.iocif  = FALSE;}

#define CAPTURE_STOP()				{timer1.on     = FALSE; \
									 timer1.tmr1   = 0;     \
									 intpie.tmr1ie = FALSE; \
									 intpir.tmr1if = FALSE; }
#define CAPTURE_RESTART(len)		{timer1.on     = FALSE;       \
									 len           = timer1.tmr1; \
									 timer1.tmr1   = 0;    \
									 timer1.on     = TRUE; \
									 intpie.tmr1ie = TRUE;}
